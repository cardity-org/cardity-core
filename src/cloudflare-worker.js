...
function runConformance(manifest, options = {}) {
  ...
  if (runtimeAdapter) {
    ...
    for (const adapterCheck of adapterValidation.checks) {
      check(`runtime_adapter.contract.${adapterCheck.id}`, "runtime_adapter", adapterCheck.status === "pass", adapterCheck.message, adapterCheck.location, adapterCheck.recommendation, adapterCheck.status === "warn");
    }
    ...
    check("runtime_adapter.action_contract", "runtime_adapter", asArray(runtimeAdapter.supported_action_contracts).includes("agent_action_contract_v1"), "Runtime adapter supports agent_action_contract_v1.", "runtime_adapter.supported_action_contracts", "Add agent_action_contract_v1 to supported_action_contracts.");
    check("runtime_adapter.projection_contract", "runtime_adapter", asArray(runtimeAdapter.supported_projection_contracts).includes("projection_contract_v1_1"), "Runtime adapter supports projection_contract_v1_1.", "runtime_adapter.supported_projection_contracts", "Add projection_contract_v1_1 to supported_projection_contracts.");
...
function validateRuntimeAdapter(adapter = {}) {
  ...
  check("adapter.schema", "schema", adapter.schema === "cardity.runtime_adapter_contract.v1", "Adapter uses cardity.runtime_adapter_contract.v1.", "schema", "Set schema to cardity.runtime_adapter_contract.v1.");
  check("adapter.runtime.name", "runtime", hasText(adapter.runtime?.name), "Adapter declares runtime.name.", "runtime.name", "Add a stable runtime name.");
  check("adapter.runtime.version", "runtime", hasText(adapter.runtime?.version), "Adapter declares runtime.version.", "runtime.version", "Add the runtime adapter version.");

  for (const field of ["supported_manifest_versions", "supported_action_contracts", "supported_projection_contracts"]) {
    check(`adapter.${field}`, "versions", asArray(adapter[field]).length > 0, `Adapter declares ${field}.`, field, `Add at least one ${field} entry.`);
  }

  const capabilities = adapter.capabilities || {};
  for (const capability of ["register_actions", "permission_gate", "dry_run_executor", "write_executor", "readback_executor", "audit_sink", "replay_guard"]) {
    check(`adapter.capability.${capability}`, "capabilities", typeof capabilities[capability] === "boolean", `Adapter declares capabilities.${capability}.`, `capabilities.${capability}`, `Set capabilities.${capability} to true or false.`);
  }

  if (capabilities.write_executor === true) {
    for (const capability of ["permission_gate", "readback_executor", "audit_sink", "replay_guard"]) {
      check(`adapter.production_write.${capability}`, "safety", capabilities[capability] === true, `Production writes require ${capability}.`, `capabilities.${capability}`, `Set capabilities.${capability}=true or disable write_executor.`);
    }
  }

  const policy = adapter.production_write_policy || {};
  check("adapter.production_write_policy.mode", "safety", ["disabled", "dry_run_only", "permissioned"].includes(policy.mode), "Adapter declares production_write_policy.mode.", "production_write_policy.mode", "Use disabled, dry_run_only, or permissioned.", true);
  if (policy.mode === "permissioned") {
    for (const field of ["requires_permission_contract", "requires_confirm_required", "requires_readback_query", "requires_idempotency_key", "requires_replay_policy"]) {
      check(`adapter.production_write_policy.${field}`, "safety", policy[field] === true, `Permissioned production writes require ${field}.`, `production_write_policy.${field}`, `Set production_write_policy.${field}=true.`);
    }
  }

  check("adapter.conformance.status", "conformance", ["untested", "passed", "failed", "partial"].includes(adapter.conformance?.status), "Adapter declares conformance.status.", "conformance.status", "Use untested, passed, failed, or partial.");
  ...
  return { schema: "cardity.runtime_adapter_validation.v1", ok: summary.failed === 0, target: adapter.runtime || null, summary, checks };
}

function renderRuntimeAdapterMarkdown(report) {
  const runtimeName = report.target?.name || "Runtime Adapter";
...
async function handleMcp(body) {
  ...
  if (body.method === "tools/call") {
    ...
    if (params.name === "cardity_validate_runtime_adapter") {
...
async function edgeApi(request) {
  ...
  if (url.pathname === "/v1/runtime-adapter/validate") {
    const report = validateRuntimeAdapter(body.runtime_adapter || body.adapter || body);
    return json({ schema: "cardity.runtime_adapter_validation_tool_result.v1", ok: report.ok, report, output: body.format === "json" ? report : renderRuntimeAdapterMarkdown(report) });
  }
  if (!body.source_text && !body.manifest && !body.old_source_text && !body.old_manifest && !body.runtime_adapter && !body.adapter && url.pathname !== "/v1/generation-guide") return null;
